CREATE OR REPLACE
    ...
    AS
    SELECT p.*,
    (SELECT COUNT(p2.id) FROM pl_laproducts p2 WHERE p2.lavariant_id = p.id) sumvariant,
    ...
    CREATE OR REPLACE
        ...
        CREATE OR REPLACE
            VIEW `pl_v_categories`
            AS
            SELECT
            c.*,
            COUNT(p.id) numproduct
            FROM pl_lacategories c
            LEFT JOIN pl_v_products p
